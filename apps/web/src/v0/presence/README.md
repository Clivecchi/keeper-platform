...
- `KeeperPresence.tsx` — schema-driven Chronicle surface; breadcrumb, related sections, relative timestamps
- `presenceEnrichment.ts` — fetches records and resolves journey context, paths, sessions per object type
- `ChroniclePresenceView.tsx` — thin Chronicle wrapper (context `chronicle`, density `standard`)
...
- **Journey** — paths section with moment counts; forward narrative
