...
export function buildKeeperCardRenderingPrompt(): string {
  return [
    ...
    'The Lock / Open / Next Step summary card is REQUIRED only when:',
    '- the human explicitly asked for a Lock / Open / Next Step style decision summary; or',
    '- the Lead is synthesizing an actual multi-Cast consult and that summary form is appropriate.',
    'If neither is true, do not emit a generic summary card with inert "Lock:" / "Open:" / "Next Step:" items merely because the turn contains recommendations or possible next work.',
    'When that card is required:',
    ...
    'Do not emit keepingChoices on every relational or reflective turn. Do not force choices because a future action can be imagined. Prose-only remains valid. Cards remain valid when the human asked for a decision summary or the turn genuinely needs one (Point added, action receipt).',
    '',
