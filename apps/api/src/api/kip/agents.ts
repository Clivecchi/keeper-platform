...
function buildDraftDiscussPrompt(
  ...
  if (intent === 'rewrite') {
    return [
      ...
      '- Use draft.point.rewrite with payload { pointId, prelude?, content? }. On journey drafts, accepted points are anchors. On document_manuscript, Lead may rewrite accepted Points. Omit payload.id on a Dialog Document.',
...
function buildGlossDiscussPrompt(
  ...
  if (anchorRaw.entityKind === 'draft' && anchorRaw.nodeId) {
    parts.push(
      ...
      '- To revise this Point, call draft.point.rewrite in THIS turn with payload { pointId, prelude?, content? } using the exact pointId above.',
...
function buildRendrDesignBoardPrompt(
  ...
  return [
    ...
    '- palette.background: paper hex (e.g. #f5f0e8)',
    '- palette.accent: identity / binding hex (e.g. #2d6a7f)',
    '- palette.ink: body type hex (optional; derived if omitted)',
    '- palette.signal: Alive / Active hex (optional; teal if omitted)',
    '- palette.action: button hex (optional; derived if omitted)',
...
/** Addressed composer owns Point writes. Nested consults and Kip Echo only advise. */
function pointTurnActorForRun(input: string, options?: RunAgentOptions | null): 'lead' | 'cast' {
  return resolvePointTurnActor({
    input,
    supportEcho: options?.supportEcho === true || isSupportEchoPrompt(input),
  });
}
...
export async function executeAgentActions(
  ...
  const expandedActions = actions.flatMap((action) => expandDraftPointRewriteActions(action));
  const normalizedActions = assignMissingRewritePointIndexes(expandedActions.map((action) => {
    if (
      ...
      const out = applyManuscriptDraftIdToProposePayload(
        normalizeDraftPointRewritePayload(p, ctx.manuscriptDraftId),
        ctx.manuscriptDraftId,
        { forceManuscript: Boolean(ctx.manuscriptDraftId) },
      );
    ...
    if (
      ...
      return {
        ...
        payload: applyManuscriptDraftIdToProposePayload(
          ...
          ctx.manuscriptDraftId,
          { forceManuscript: isKipDraftUuid(ctx.manuscriptDraftId) },
        ),
      };
    }
    if (
      ...
      try {
        ...
        switch (action.type) {
          ...
          case 'draft.update.propose': {
            ...
            let draft = await findDraftForPointMutation(tx, draftId, ctx);
            if (
              draft?.kind === 'document_manuscript'
              && ctx.dialogId
              && draft.dialog_id
              && draft.dialog_id !== ctx.dialogId
            ) {
              const retargetId = await resolveManuscriptDraftIdForPropose(ctx);
              if (retargetId && retargetId !== draft.id) {
                const retargeted = await findDraftForPointMutation(tx, retargetId, ctx);
                if (retargeted) draft = retargeted;
              }
            }
            if (!draft) {
          ...
          case 'draft.point.rewrite': {
            ...
            const pointRef =
              coercePointRewriteRef(payload)
              ?? (typeof payload.id === 'string'
                && payload.id.trim()
                && payload.id.trim() !== draftId
                ? payload.id.trim()
                : '');
            const content = typeof payload.content === 'string' ? payload.content.trim() : '';
            const title =
            ...
            if (!draftId) {
              results.push({
                type: action.type,
                status: 'error',
                message: 'Draft id required for rewrite — omit it on a Dialog Document and Keeper fills the manuscript.',
                errorCode: 'VALIDATION_ERROR',
              });
              break;
            }
            if (!pointRef) {
              results.push({
                type: action.type,
                status: 'error',
                message: 'Point id required for rewrite — use 1–N from DIALOG DOCUMENT, the current title, or an exact id.',
                errorCode: 'VALIDATION_ERROR',
              });
              break;
            }
            if (!content && !title) {
              results.push({
                type: action.type,
                status: 'error',
                message: 'Provide a new title (prelude) and/or new content.',
                errorCode: 'VALIDATION_ERROR',
              });
              break;
            }
            ...
            const resolvedPointId = resolveDraftPointRef(pointRef, parseDraftPoints(draft.spec_json), {
              prelude: title || undefined,
              content: content || undefined,
            });
            if (!resolvedPointId) {
              const hostCount = parseDraftPoints(draft.spec_json).filter(
              ...
              results.push({
                ...
                message:
                  hostCount > 0
                    ? `Point not found. Use 1–${hostCount} from DIALOG DOCUMENT, the current Point title, or an exact id.`
                    : 'Point not found — this Document has no Points to rewrite.',
              ...
              const message =
                rewriteResult.code === 'POINT_ANCHORED'
                  ...
                  : rewriteResult.code === 'POINT_NOT_FOUND'
                    ? 'Point not found. Use 1–N from DIALOG DOCUMENT, the current Point title, or an exact id.'
                    : 'Provide a new title (prelude) and/or new content.';
              results.push({
                type: action.type,
                status: 'error',
                message,
  ...
  static async callAIModel(
    ...
    try {
      ...
      if (environmentContext) {
        ...
        if (pointObligation) {
          const envSupportEcho =
            (environmentContext as { agentContext?: { supportEcho?: boolean } } | null | undefined)
              ?.agentContext?.supportEcho === true;
          const actor = resolvePointTurnActor({
            input,
            supportEcho: isSupportEchoPrompt(input) || envSupportEcho,
          });
    ...
    try {
      ...
      if (agent.role === 'Lead') {
        ...
        if (
          ...
          options?.onStatus?.('Rewriting Point titles…');
          options?.onReset?.();
          const dialogTitle =
            (options?.environment as { dialogDocument?: { title?: string } } | undefined)
              ?.dialogDocument?.title;
          const rewriteFollowUpInput = buildPointRewriteFollowUpInput({
            originalInput: input,
            agentName: agent.name,
            dialogTitle,
            priorResponseText: finalResponseText,
          });
          const rewriteFollowUpResult = await this.callAIModel(
            agent,
            rewriteFollowUpInput,
            previousMessages,
            userId,
            {
              mode: activeMode,
              ...
              attachments: options?.attachments ?? undefined,
              timings: options?.timings,
              timingLabel: 'point_rewrite_follow_up',
              reuseMessages: [
                ...lastPromptMessages,
                { role: 'assistant', content: finalResponseText },
              ],
          ...
          lastPromptMessages = rewriteFollowUpResult.messages;
          const rewriteFollowUpStructured = await ensureKipAgentOutputEnvelope(
            rewriteFollowUpResult.content,
            {
              requestId: randomUUID(),
              userId,
              allowedActions: Array.from(allowActions),
          ...
          finalResponseText =
            rewriteFollowUpStructured.responseText?.trim()
            || rewriteFollowUpResult.content.trim()
            || finalResponseText;
          if (options?.onDelta && !rewriteFollowUpResult.streamedVisible && finalResponseText.trim()) {
            options.onDelta(finalResponseText);
          }
          if (rewriteFollowUpStructured.actions.length) {
            const rewriteActionsStartedAt = Date.now();
            const rewriteFollowUpExecution = await executeAgentActions(
              rewriteFollowUpStructured.actions,
              buildExecuteAgentActionsCtx(options, {
                userId,
                agentId: agent.id,
                allowlist: allowActions,
            ...
            if (options?.timings) {
              options.timings.actionsMs =
                (options.timings.actionsMs ?? 0) + (Date.now() - rewriteActionsStartedAt);
            }
            actionResults = [...actionResults, ...rewriteFollowUpExecution.results];
            if (rewriteFollowUpExecution.failedMessage) {
              finalResponseText = `${finalResponseText}\n\n${rewriteFollowUpExecution.failedMessage}`;
            }
          }
        }
      ...
      // Handle Coordinator agents
        ...
        if (
          ...
          lastSystemPromptMessages = followUpResult.messages;
          const followUpStructured = await ensureKipAgentOutputEnvelope(followUpResult.content, {
            requestId: randomUUID(),
            userId,
            allowedActions: Array.from(systemAllowActions),
          ...
          if (followUpStructured.actions.length) {
            ...
            if (
              followUpExecution.failedMessage
              && !(systemPointObligation?.required === true && !systemPointObligation.constrained)
            ) {
              finalResponseText = `${finalResponseText}\n\n${followUpExecution.failedMessage}`;
            }
          }
        ...
        if (systemObligationFollowUp || systemPointAskFollowUp) {
          ...
          lastSystemPromptMessages = pointFollowUpResult.messages;
          const pointFollowUpStructured = await ensureKipAgentOutputEnvelope(pointFollowUpResult.content, {
            requestId: randomUUID(),
            userId,
            allowedActions: Array.from(systemAllowActions),
        ...
        if (systemPointObligation) {
          const blockedNotice = buildPointObligationBlockedNotice(systemPointObligation);
          if (blockedNotice) {
            finalResponseText = `${finalResponseText}\n\n${blockedNotice}`;
          } else if (
            pointTurnActor === 'lead'
        ...
        if (
          systemPointObligation?.required
          && !systemPointObligation.constrained
          && pointTurnActor === 'cast'
        ) {
        ...
        if (systemPointObligation) {
          ...
          const pointCard = buildPointTurnCard({
            results: actionResults,
            dialogTitle: systemPointObligation.dialogTitle,
            workingOnTitle: systemPointObligation.workingOnTitle,
            writeKind: systemPointObligation.writeKind,
          });
          if (pointCard) {
            structured = { ...structured, card: pointCard };
            const presented = applyPointTurnDialogCopy({
              ...
              results: actionResults,
              dialogTitle: systemPointObligation.dialogTitle,
              obligationRequired: systemPointObligation.required === true && !systemPointObligation.constrained,
            });
            if (presented !== finalResponseText.trim()) {
              finalResponseText = presented;
              options?.onReset?.();
        ...
        if (action === 'run') {
          ...
          try {
            ...
            environment = await resolveAgentEnvironment({
              ...
              chronicleDraftId: validation.data.activeDraftId,
              intent: 'interactive',
            });
          ...
          if (environment) {
            if (validation.data.activeDraftId === null) {
              environment.activeDraft = undefined;
            }
            let draftIdToEnrich =
            ...
            if (
              !draftIdToEnrich
              && validation.data.activeDraftId !== null
              && !validation.data.dialogId
              && sessionId
            ) {
              try {
