...
- 2026-02-08: Fixed inline `setSessionCookie` in `index.ts` — changed `SameSite=Lax` to `SameSite=None` and made `Secure` unconditional. The 2025-10-15 fix was only applied to `session.ts` but never propagated to the inline handler that actually runs for login/register. This was the root cause of persistent 401 errors.
