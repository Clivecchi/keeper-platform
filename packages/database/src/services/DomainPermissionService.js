...
export class DomainPermissionService {
     ...
     */
    async revokePermission(domainId, userId, revokedBy) {
    ...
    async getUserPermissions(userId) {
        ...
        if (cachedDomainIds) {
            const permissions = await Promise.all(cachedDomainIds.map(async (domainId) => {
    ...
    async getDomainUsers(domainId) {
        ...
        if (cachedUserIds) {
            const users = await Promise.all(cachedUserIds.map(async (userId) => {
                const permission = await this.cacheService.getUserPermissions(userId, domainId);
                if (permission && !this.isPermissionExpired(permission)) {
                    return {
                        userId,
        ...
        for (const [key, userChecks] of uniqueChecks) {
            ...
            userChecks.forEach(check => {
                ...
                if (basePermission.source === 'none') {
                    ...
                    const hasSpecificPermission = this.hasPermissionInList(check.permission, basePermission.permissions);
     ...
     */
    async updatePermissionExpiration(domainId, userId, expiresAt, updatedBy) {
     ...
     */
    evaluatePermission(permission, requestedPermission) {
        ...
        const hasPermission = this.hasPermissionInList(requestedPermission, permission.permissions);
        return {
            hasPermission,
            role: permission.role,
            permissions: permission.permissions,
    ...
    hasPermissionInList(permission, permissions) {
    ...
    validatePermissionsForRole(role, permissions) {
     ...
     */
    permissionToSummary(permission, isExpired) {
        return {
     ...
     */
    async getPermissionStats(domainId) {
     ...
     */
    async getDomainPermissions(domainId, userId) {
        ...
        if (domain.ownerId === userId) {
            result.unshift({
                userId: domain.ownerId,
                role: 'admin',
                permissions: ['read', 'write', 'admin', 'share', 'delete'],
...
//# sourceMappingURL=DomainPermissionService.js.map