...
export class FeatureFlagService {
    ...
    getFlagsByEnvironment(environment) {
        return Array.from(this.flags.values()).filter(flag => flag.environments.includes(environment));
    }
    /**
     * Get rollout status for a flag
     */
    getRolloutStatus(flagKey) {
        const flag = this.flags.get(flagKey);
        if (!flag) {
            return null;
        }
     ...
     */
    toggleFlagsByCategory(category, enabled) {
...
export const domainFlags = {
    isDomainLayerEnabled: (context) => getFeatureFlagService().isEnabled('DOMAIN_LAYER_ENABLED', context),
    isCustomDomainsEnabled: (context) => getFeatureFlagService().isEnabled('CUSTOM_DOMAINS_ENABLED', context),
    isDomainVerificationEnabled: (context) => getFeatureFlagService().isEnabled('DOMAIN_VERIFICATION_ENABLED', context),
    isCrossDomainSharingEnabled: (context) => getFeatureFlagService().isEnabled('CROSS_DOMAIN_SHARING_ENABLED', context),
    isSoleDomainIsolationEnabled: (context) => getFeatureFlagService().isEnabled('SOLE_DOMAIN_ISOLATION_ENABLED', context),
    isDomainCachingEnabled: (context) => getFeatureFlagService().isEnabled('DOMAIN_CACHING_ENABLED', context),
    isDomainAdminPanelEnabled: (context) => getFeatureFlagService().isEnabled('DOMAIN_ADMIN_PANEL_ENABLED', context),
    isPremiumDomainFeaturesEnabled: (context) => getFeatureFlagService().isEnabled('PREMIUM_DOMAIN_FEATURES_ENABLED', context),
};
