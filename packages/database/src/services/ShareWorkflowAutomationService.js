...
export class ShareWorkflowAutomationService {
     ...
     */
    async initializeWorkflowExecution(requestId, workflowId, context = {}) {
        ...
        const stepExecutions = await Promise.all(workflow.workflowSteps.map(async (step, index) => {
     ...
     */
    async executeStep(stepExecutionId) {
     ...
     */
    async completeApprovalStep(stepExecutionId, approvedBy, comments, appliedRules) {
        const stepExecution = await this.prisma.shareStepExecution.findUnique({
            where: { id: stepExecutionId },
     ...
     */
    async progressToNextStep(requestId, currentStepNumber) {
     ...
     */
    async completeWorkflow(requestId) {
        ...
        if (request?.workflow?.autoApprovalRules && Array.isArray(request.workflow.autoApprovalRules)) {
            const autoApprovalRules = request.workflow.autoApprovalRules;
            if (autoApprovalRules.some((rule) => rule.autoActivate)) {
    ...
    async validateCondition(condition, shareRequest, stepExecution) {
        ...
        switch (condition.type) {
            case 'user_role':
                const userRole = await this.getUserRole(shareRequest.requestedBy, shareRequest.sourceDomainId);
                actualValue = userRole;
                isValid = this.evaluateCondition(userRole, condition.operator, condition.value);
                break;
            case 'domain_property':
                const domainProperty = await this.getDomainProperty(shareRequest.sourceDomainId, condition.field);
            ...
            case 'approval_count':
                ...
                break;
            case 'custom':
                const customResult = await this.evaluateCustomCondition(condition, shareRequest, stepExecution);
     ...
     */
    evaluateTimeCondition(actual, condition) {
    ...
    async getWorkflowMetrics(workflowId, days = 30) {
        ...
        const autoApprovalCount = executions.filter(e => e.stepExecutions.some(s => s.approvedBy === 'system')).length;
        const autoApprovalRate = totalExecutions > 0 ? autoApprovalCount / totalExecutions : 0;
        // Calculate timeout rate
        const timeoutCount = executions.filter(e => e.stepExecutions.some(s => s.status === 'timeout')).length;
    ...
    async getDomainProperty(domainId, property) {
        const domain = await this.prisma.domain.findUnique({
            where: { id: domainId },
        });
        return domain ? domain[property] : null;
    }
    ...
    async handleStepTimeout(stepExecutionId) {
        await this.prisma.shareStepExecution.update({
            where: { id: stepExecutionId },
            data: {
                status: 'timeout',
    ...
    async handleStepError(stepExecutionId, error) {
        const stepExecution = await this.prisma.shareStepExecution.findUnique({
            where: { id: stepExecutionId },
        });
...
//# sourceMappingURL=ShareWorkflowAutomationService.js.map