...
export class DomainVerificationService {
     ...
     */
    async initiateVerification(domainId, customDomain, method = 'DNS_TXT') {
    ...
    async verifyDomain(domainId) {
        ...
        const method = domain.verificationMethod;
        let result;
        try {
            switch (method) {
                case 'DNS_TXT':
     ...
     */
    async verifyDnsTxt(customDomain, expectedToken) {
        try {
            const txtRecords = await dnsResolveTxt(`_keeper-verification.${customDomain}`);
            const flatRecords = txtRecords.flat();
            const hasValidRecord = flatRecords.some(record => record.includes(`keeper-site-verification=${expectedToken}`));
            return {
     ...
     */
    async verifyDnsCname(customDomain) {
        try {
            const cnameRecords = await dnsResolveCname(customDomain);
            const pointsToKeeper = cnameRecords.some(cname => this.KEEPER_DOMAINS.some(keeperDomain => cname.toLowerCase().endsWith(keeperDomain.toLowerCase())));
            return {
     ...
     */
    async verifyHttpFile(customDomain, expectedToken) {
        try {
     ...
     */
    async verifyMetaTag(customDomain, expectedToken) {
        try {
     ...
     */
    async checkDomainHealth(customDomain) {
     ...
     */
    async generateVerificationConfig(method, customDomain) {
     ...
     */
    async checkDnsResolution(customDomain) {
        const result = {
            hasARecord: false,
        ...
        try {
            ...
            result.pointsToKeeper = result.pointsToKeeper || cnameRecords.some(cname => this.KEEPER_DOMAINS.some(keeperDomain => cname.toLowerCase().endsWith(keeperDomain.toLowerCase())));
        }
        catch (error) {
     ...
     */
    async checkHttpEndpoints(customDomain) {
        const result = {
     ...
     */
    async checkSslCertificate(customDomain) {
        try {
    ...
    async checkDomainAvailability(customDomain, excludeDomainId) {
    ...
    async logVerificationAttempt(domainId, method, result) {
        try {
     ...
     */
    async batchVerifyDomains(domainIds) {
...
//# sourceMappingURL=DomainVerificationService.js.map