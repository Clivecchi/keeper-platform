...
| 2026-07-22 | **Live-testing punch list:** four gaps found post-Phase-1, all shared across board presets. (1) Eager Dialog on mount — shipped above. (2) Realm's `/home` Chronicle never reaches the real Document renderer — `realm-home-chronicle-routing`, queued. (3) Kip's own agent-availability check doesn't read `DialogCastMember` — `kip-roster-dialog-cast-sync`, queued. (4) Nav duplication fixed directly. Full detail in `docs/chronicle-document-architecture.md`. |
