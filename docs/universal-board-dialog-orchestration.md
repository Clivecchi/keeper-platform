...
| Date | Change |
|---|---|
| 2026-07-22 | **Live-testing punch list:** four gaps found post-Phase-1, all shared across board presets. (1) Every board eagerly creates a Dialog+session on mount, not first message — `stop-eager-dialog-creation`, now current. (2) Realm's `/home` Chronicle never reaches the real Document renderer — `realm-home-chronicle-routing`, queued. (3) Kip's own agent-availability check doesn't read `DialogCastMember`, so it contradicts what the Cast Header already shows — `kip-roster-dialog-cast-sync`, queued. (4) Nav duplication (dialog name shown twice) fixed directly. Full detail in `docs/chronicle-document-architecture.md`. |
