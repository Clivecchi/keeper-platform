...
**2. Every board eagerly creates a Dialog + session on mere mount, not on first message.** Chuck visited the IDE board, sent nothing, and it persisted a Dialog titled "Ide · conversation · Jul 23" — which then polluted Realm's Nav. Root cause: `UniversalConversation.tsx:1720` (IDE) and `:1774` (Designer), plus `useAgentDialog.ts:482-551` (Agent/Domain/Realm) and a second parallel path at `:553-620`, all funnel through `resumeOrCreateBoardSession` → `findOrCreateKipDialog` (`kipDialogLifecycle.ts:36-93`), which unconditionally `prisma.dialog.create()`s the moment no existing session is found — gated on mount, not on send. One root cause, four call sites, shared across every preset. Scoped as `stop-eager-dialog-creation` (**now current** — first in line because it's the most bounded and mechanical, and because it's actively writing garbage into production until it ships).

**3. Chronicle doesn't look like Document on `/home` — because it isn't rendering at all there.** `RealmHomeChronicle.tsx:77` only reaches the real Document renderer (`DomainRealmStory`, already correct, not hardcoded to one dialog) `if (!useUserFeed && ...)` — i.e. only when `isUserHome` is false. `V0Shell.tsx:838,840` sets `shellMode: "home"` for the `/home` route regardless of `?board=realm`, so `/home?board=realm` always takes the other branch and falls through to an empty `aria-hidden` div (`RealmHomeChronicle.tsx:88-90`) whenever the feed has no events. Confirmed *not* a CSS/width bug — `KeeperBoardPanelGroup.tsx` still allocates the panel real space — and *not* the old hardcoded-dialog theory either. Never finished, not broken. Scoped as `realm-home-chronicle-routing` (queued, second in line).

**4. Ceox "exists but doesn't work" — the cast-bar display and Kip's own availability check are two different code paths, only one of which knows about `DialogCastMember`.** The display path (`listDialogCastMembers`, `dialogCastMembership.ts:157-201`) is correct and is the only place that reads the new table. Kip's own roster — injected into its system prompt at `agents.ts:3903-3922` — comes from `resolveAgentEnvironment.ts`, which has **no `dialogId` parameter at all** and only ever calls `loadDomainScopedAgents` (the domain-level baseline). The literal phrases "Agent Availability" / "not part of the team" aren't hardcoded anywhere — Kip is an LLM correctly following its own instruction *("do not claim domain agents are absent when they appear in this list")* against a list that genuinely doesn't include Ceox yet. Scoped as `kip-roster-dialog-cast-sync` (queued, third in line — least urgent of the three, since it's a wrong sentence from Kip rather than data pollution or a missing panel, but it's the fix that would make Phase 1 actually *feel* complete).

All three follow-on handoffs are fully scoped (`docs/build-handoffs/current.json` for #2; `docs/build-handoffs/archive/2026-07-22-realm-home-chronicle-routing-queued.json` and `.../2026-07-22-kip-roster-dialog-cast-sync-queued.json` for #3 and #4) so each can become `current` in turn without re-deriving this investigation. Real per-agent delegation (Phase 2, `director-lead-initiated-delegation`) remains after all three, unchanged from the 2026-07-22 decision above.
