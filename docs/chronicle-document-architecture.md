...
**Chuck's own model, verbatim, is the fix:** *"a User that is involved in a dialog is able to enable lead agents from any domain they themselves have Admin privilege over."* Kip/Cloud/Rendr are ke3p's own domain agents; Ceox is Chuck's own agent, portable wherever he has real Admin `DomainPermission` — not attached to one domain's hardcoded list. Scoped as `cross-domain-cast-membership`: resolve a user's admin domains, resolve each one's lead agent (reusing the existing `resolveDomainLeadContext` pattern, not reinventing it), surface them as addable via a real "Add" action in `DirectorCastHeader`, gated server-side on real `DomainPermission`, never trusted from the client.
