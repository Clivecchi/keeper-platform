...
**Status:** Layer 1 shim + Layer 2 registry (Library pilot) + Document/Point reconciliation landed on `cloud` (`Point` atomic / `Document` container; Dialog.document_status; Point→Moment identity keep; DocumentShell; Dialog DELETE; Forward/Step header). Layer 3 unchanged (`resolveChronicleDeclaration.ts`). `realm-becoming-together-parity` (rendering) and `ke3p-becoming-together-consolidation` (data) shipped 2026-07-20. Two follow-on bugs found and fixed directly the same day: archive-tagged Library items were leaking into Nav's Kept bucket unfiltered (fixed), and ke3p kept spawning new dialogs because Becoming Together's binding didn't match how real sessions get found/reused (fixed — ke3p is back to exactly 1 active dialog). `realm-director-mode-unification` shipped 2026-07-20 — shared `BoardInstrumentsBar` Agents roster on Realm / Domain / IDE / Designer; Realm header cast retired to trailing access actions. Cursor's version dropped Cloud/Rendr from Realm's bar (domainDirectorBoardInstruments still excluded them via old platformComposerSlugs logic instead of using a declared boardInstruments list like IDE/Designer) — fixed directly same day, verified by diff. Also dropped, not yet decided: the personal-agent (Ceox) member-representation concept that used to live in DialogCastBar has no replacement in the new shared bar — real open question, not silently resolved either way. `realm-header-cast-and-multiselect` is now current — cast identity moves to a universal header, invocation stays at the composer, Kip is active by default, and Domain/Realm gains true multi-select instrument engagement (IDE/Designer keep single-select). Correction: an earlier report wrongly framed Library's lack of a Dialog relationship as a gap — Library is intentionally domain-wide per canon (`Domain → LibraryItem`), not hierarchical; the 47+8 items in Realm's Kept/Presented Nav are real, separate, pre-existing content unrelated to the ke3p consolidation, and their undifferentiated presentation is the long-parked Curator/grouping work, not a bug. `draft-renders-as-document` remains re-parked behind this.
