...
**Shipped:** confirmed landed in commit `c6ae3076` ("feat: add Forward and Step header to DocumentShell"), verified against real `DocumentShell.tsx`/`document.ts` on 2026-07-19.
...
## Shipped verification (2026-07-19)

Confirmed against real code, not just the commit message: `DocumentShell.tsx` and `document.ts` contain every element above (forward/step props, collapse logic, glass Step treatment on `--theme-status-success`, disabled Back/Forward with the exact tooltips, correct brightness hierarchy). `buildGroups`/`PathHeader`/`PointFrame` untouched, as required. One gap: `pnpm run quick:web` currently fails, but on pre-existing errors in unrelated files (`ChronicleActPresence.tsx`, `serviceConfig.tsx`, `presenceEnrichment.ts`, `useRealmNavGrowth.ts`, `V0Shell.tsx`, `StyleOverrideProvider.tsx`, a test file) — zero errors in either file this handoff touched.

Real-code check also caught a documentation-only nit: the architecture doc's own "lesson" section describes the Step title color as matching a `--moss` token, but no such token exists anywhere in the codebase (verified by grep) — the shipped component correctly uses the real, existing `--theme-status-success` token instead. Doc detail was slightly off; the shipped code is correct.
