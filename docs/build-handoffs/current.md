...
Separately noted: `pnpm run quick:web` currently fails with roughly 108 pre-existing errors across ~33 files repo-wide (re-verified 2026-07-19 against full output, not truncated) — none in this handoff's touch list as of this writing. Two commits landed on `cloud` during this same session that this handoff's author didn't make (`ab9b7101 "Forward"`, `2c5123b0 "Forward 2"`) — the second already rewrote `useRealmNavGrowth.ts` for N+1-fetch performance (`dialogId`/`pointIds` now come straight off the drafts list endpoint, no per-draft detail fetch) and added request dedupe since Nav and Chronicle both call the hook. Re-read the file's current state before touching it — this handoff's assumptions about its exact contents may be stale relative to whatever lands next. Do not fix the pre-existing repo-wide type errors as part of this handoff, just don't add new ones.
