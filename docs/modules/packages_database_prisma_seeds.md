...
- TS seeds run via `prisma/seed.ts` to insert platform roles, domains, boards, engagement templates, Kip lenses (Domain Lens, Debug Investigator Lens for `default`), and default domain policy packs (`policy-v1`) for every domain.
