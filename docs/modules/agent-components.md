...
## Update Log
- 2026-05-26 (Agent Board Phase 4 — Kip Echo): `types.ts` — added `DialogResponseEcho` and optional `echo` on `AgentDialogueMessage`. `DialogueMessageList.tsx` — renders Echo as subordinate beat beneath agent message (Kip attribution, smaller type, no avatar). Primary agent bubble rendering unchanged.
