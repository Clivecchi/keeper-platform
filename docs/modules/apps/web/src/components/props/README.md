...
### 2026-08-09: **LinkedCard affordance follows destination**
- `LinkedCard` only looks interactive when `onNavigate` is set or `href` is a real destination (not empty / `#`).
- Static cards render as muted non-interactive `div`s — no hover/press ring.
