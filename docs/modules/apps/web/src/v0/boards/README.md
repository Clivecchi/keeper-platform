...
- `directorDialog.sanitizeAgentMessageContent` unwraps `agent_output` JSON even when it is pasted under a Cast heading.
...
- Added `cueingLabel` prop to `KeeperDialogFrame` (passed through to `DirectorCastHeader`) — shows "Cueing: Directed" / "Cueing: Monologue" / etc. next to the header cast eyebrow, derived from `def.conversation.dialogCueing`.
- `useAgentDialog.ts` was already on the new vocabulary (`cuedCastSlugs`, `castLabels`, `activeCastMember`, `resolveCastParticipation`, `resolveDirectorCastMember`, `buildCastDelegationPrompt`, phase `"cast"`); no further changes needed. Wire shape to the API (`castConsultations.consultations[].instrumentSlug`/`instrumentReply`, `directorDelegation.instrumentSlug`/`instrumentReply`) intentionally kept as-is — `apps/api/src/api/kip/agents.ts`'s `castConsultations` schema only accepts those legacy keys.
- `pnpm run quick:web` type-check confirmed clean for these three files (`UniversalConversation.tsx`, `useAgentDialog.ts`, `KeeperDialogFrame.tsx`) — all remaining `tsc` errors in `UniversalConversation.tsx` (missing `KipMessage`/`extractAgentReplyFromRunResult` imports, `DomainFrameJson`/`DomainFrameTreatment` cast mismatches) are pre-existing and unrelated to this rename.
