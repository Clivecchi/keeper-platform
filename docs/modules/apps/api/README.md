...
## 🔄 Data & Behavior
Runs Prisma-backed endpoints, CORS hardened for single-domain MVP. Origins are driven by `PUBLIC_WEB_ORIGIN` and `APP_ORIGIN`. Tenant web hosts on `https://{slug}.keeper.domains` are allowed via regex-style validation (reserved infra subdomains excluded). Domain resolution middleware is present but routes remain platform-scoped for MVP.
...
- **Environment Variables**: PORT (defaults to 3001), `DATABASE_URL` (runtime / PgBouncer), `DIRECT_URL` (migrate), optional `PRISMA_CONNECTION_LIMIT` (default 5)
- **Database access**: always `import { prisma } from '@keeper/database'` — never `new PrismaClient()` (one pool per process)
