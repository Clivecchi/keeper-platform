...
- 2026-02-08: Updated `domainResolutionMiddleware.ts` — reordered `resolveDomain` to check `isPlatformBaseDomain` BEFORE `isReservedHostname`, preventing `api.ke3p.com` from being blocked. Changed `isReservedHostname` to return a no-domain-context result instead of a hard error, so requests continue to next middleware. Added `ke3p.com`, `www.ke3p.com`, `api.ke3p.com` to platform base domains list. Added `API_BASE_URL` env var support for platform domain recognition.
- 2026-02-08: Updated `domainPermissionMiddleware.ts` — added `ensureDomainContext()` helper that resolves domain from `req.params.domainId` when domainContext is not set (hostname resolution skipped). Updated `requireDomainReadCompat`, `requireDomainWriteCompat`, `requireDomainAdminCompat`, `requireDomainOwnershipCompat`, `loadDomainPermissionsCompat` to call `ensureDomainContext` before permission checks. Permission resolution checks domain ownership, DomainPermission table, keeper relationships, with fallback to read access for authenticated users.
...
- Extracted `resolveParamDomainAccess` with tests.
