...
## 🔄 Data & Behavior
- **Cover (Integration + Key):** `IntegrationFocusPresence` / `KeyFocusPresence` always render `DeclarationChronicleBlocks` below `EntityCoverPresence`; client-side defaults from `@keeper/shared` when DB `chronicle_blocks` is empty (`resolveChronicleDeclaration.ts`)
