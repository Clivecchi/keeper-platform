...
- `resolveKindId` includes `dialog` when `selectedDialogId` is set — Trail Bar shows Dialog; Panel Body renders idle (no Chronicle Dialog view yet).
- `DialogIdleView` resolves dialog title for trail labels; `UniversalViewPanelIdle` recent moments wire `onMomentSelect`.
- `KeeperView` uses domain keeper list (avoids broken direct `/api/keepers/:id` domain context); shows loading until `domainId` resolves.
- `DraftView` uses `KipApi.getDraft`; loading state until `domainId` resolves.
- `JourneyView` path labels use `Name · N moments` format; `MomentView` title primary with full narrative below.
